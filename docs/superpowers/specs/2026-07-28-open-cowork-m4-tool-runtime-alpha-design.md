...
完成信号沿用 `M4-ACC-001` 至 `M4-ACC-010`：八项以自动化、故障注入和现有真实
平台证据关闭，`M4-ACC-006`、`M4-ACC-009` 的 Windows 真机维度明确延期。不得以
契约存在代替固定管线和副作用不重复证据，也不得把延期改写为通过。
...
关闭时 `M4-ACC-006`、`M4-ACC-009` 已具备自动化故障注入和 `osx-arm64` 真机证据，
但 `win-x64` 真机证据尚未执行，因此两项标记为 `Deferred`；其后续清单和平台状态
以双平台真机发布验证台账为准。
...
Node REPL、第二套工具状态机或独立 Tool Store。按 2026-07-28 用户确认的关闭边界，
M4 在八项验收通过、两项 Windows 真机维度明确延期且未误报为通过后标记完成；
延期证据必须在双平台台账中持续保留，并在 M10 发布候选关闭前补齐。

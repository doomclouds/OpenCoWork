...
完成信号沿用 `M4-ACC-001` 至 `M4-ACC-010`：十项现均有自动化、故障注入或
双平台真机证据。不得以契约存在代替固定管线和副作用不重复证据。
...
2026-07-28 初次关闭时，`M4-ACC-006`、`M4-ACC-009` 因缺少 `win-x64` 真机证据
标记为 `Deferred`。2026-07-29 Windows 补验完成，两项已改为 `Passed`；验证基线、
环境和场景以双平台真机发布验证台账为准。

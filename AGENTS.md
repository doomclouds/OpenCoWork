...
- 日常开发、计划执行和普通提交统一在 `dev` 分支进行；开始任务前先确认当前分支为 `dev`。
- `main` 仅接收已验收、经用户明确要求集成的稳定结果。
...
`win-x64` 与 `osx-arm64` 的已验证基线、待验证项、证据入口和更新规则统一记录在
`docs/platform-release-validation-ledger.md`。开始或关闭任何要求双平台证据的
Slice 前必须先读取该台账；交叉发布只能证明产物可生成，不能替代对应平台真机运行。
`AGENTS.md` 不重复保存具体结果。
